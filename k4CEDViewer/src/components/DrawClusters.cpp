...
struct DrawClusters final : k4FWCore::Consumer<void(const edm4hep::ClusterCollection&)> {
  ...
  Gaudi::Property<bool> colorEnergy{this, "ColorByEnergy", false, "color recunstructed particle by energy"};

  Gaudi::Property<bool> colorEnergyAuto{
  ...
  void operator()(const edm4hep::ClusterCollection& col) const override {
    ...
    info() << " +++++++  drawing Cluster collection with " << col.size() << " particles "
    ...
    for (auto cluster : col) {
      ...
      if (colorEnergy) {
        if (colorEnergyAuto) {
          ...
          color = ColorMap::NumberToTemperature(ene, colorEnergyMin, colorEnergyMax, colorEnergySaturation,
                                                colorEnergyValue);
        }
      }
      ...
      if (drawEllipsoidForPFOClusters) {
        ...
        // For each cluster, it's (energy weighted) central position, the deposited energy and the intrinsic direction
        // in terms of sperical angles are given. The minimal and maximal deposited energies among all clusters in the
        // displayed event have been determined previously and will be needed for coloring.
        ...
        for (int i = 0; i < 3; i++)
          for (int j = 0; j < 3; j++)
            I[i][j] = 0;
        // The angles theta and phi are used to transform the coordinates of each hit into a c.s. in which the x'-axis
        // is parrallel to the ellisoid's intrinsic direction. Note that this does not describe an unambigious system as
        // any rotation along the x'-axis does not touch this constraint. The transformation is achieved by a typical
        // multiplication of rotation matrices: R(theta, phi) = R_y(Pi/2 - theta)*R_z(phi)
        double R[3][3];
        ...
        for (int q = 0; q < nHits; q++) {
          ...
          // ced_hit_ID(x,y,z,marker, layer ,size,color,myColID + part.id().index);   //this line draws the indivdual
          // hits within a cluster translation and rotation of the coordinates
          x -= cluster_center[0];
          y -= cluster_center[1];
          z -= cluster_center[2];
          tot_x += x * e;
        ...
        //                           y'                                      <-->        y
        //                           z'                                      <-->       -x
        // These assignments are corrected for by a modified rotation of the ellipsoid along its y-axis (see declaration
        // of double rotate[])
        ...
        if (nHits > 0) {
          ...
          double lambda1 =
              0.5 * (I[2][2] + I[1][1]) + sqrt(pow(0.5 * (I[2][2] + I[1][1]), 2) + pow(I[2][1], 2) - I[2][2] * I[1][1]);
          double lambda2 =
              0.5 * (I[2][2] + I[1][1]) - sqrt(pow(0.5 * (I[2][2] + I[1][1]), 2) + pow(I[2][1], 2) - I[2][2] * I[1][1]);
          sizes[0] = I[0][0];
          sizes[1] = lambda1;
        ...
        int ellipsoid_color =
            color; //( colorByEnergy ? returnRGBClusterColor(cluster.getEnergy(), Emin, Emax, 256, 'a', 3) : color ) ;
