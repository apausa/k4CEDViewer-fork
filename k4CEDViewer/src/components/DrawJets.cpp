...
struct DrawJets final : k4FWCore::Consumer<void(const edm4hep::ReconstructedParticleCollection&)> {
  DrawJets(const std::string& name, ISvcLocator* svcLoc) : Consumer(name, svcLoc, KeyValue("colName", {"Jets"})) {
  ...
  void operator()(const edm4hep::ReconstructedParticleCollection& col) const override {
    ...
    int color = int(RGBAcolor[2] * (15 * 16 + 15)) + int(RGBAcolor[1] * (15 * 16 + 15)) * 16 * 16 +
                int(RGBAcolor[0] * (15 * 16 + 15)) * 16 * 16 * 16 * 16;
    ...
    for (auto jet : col) {
      ...
      float pt_tot = 0.0;
      float E_max = 0.0;
      float mean_tan_angle = 0.0;
      std::vector<TVector3> pp;
      std::vector<float> pt;
      std::vector<float> E;
      pp.reserve(N_elements);
      pt.reserve(N_elements);
      E.reserve(N_elements);
      ...
      for (int k = 0; k < N_elements; ++k) {
        ...
        double momLength =
            (E[k] / E_max) * calculateTrackLength(ecalBarrelParams, ecalEndcapParams, center_ref[0], center_ref[1],
                                                  center_ref[2], pp[k].X(), pp[k].Y(), pp[k].Z(), 0); // line size
        // approximation: all lines start in origin (TODO, if jet origin known)
        ced_line_ID(center_ref[0], center_ref[1], center_ref[2], momLength * pp[k].X() / pp[k].Mag(),
                    momLength * pp[k].Y() / pp[k].Mag(), momLength * pp[k].Z() / pp[k].Mag(), layer, 1, color,
                    pv[k].id().index);
      }
      ...
      double coneHeight = calculateTrackLength(ecalBarrelParams, ecalEndcapParams, center_c[0], center_c[1],
                                               center_c[2], v.X(), v.Y(), v.Z(), 0);
      // 1. baseline radius, 2. height, 3. origin doublet, 4. rotation triplet,...
      ced_cone_r_ID(mean_tan_angle * coneHeight, coneHeight, center_c, rotation_c, layer, RGBAcolor,
                    myColID + jet.id().index);
    }
