...
struct DrawTracks final : k4FWCore::Consumer<void(const edm4hep::TrackCollection&)> {
  ...
  Gaudi::Property<int> drawHelixForTracks{
      this, "drawHelixForTracks", 1,
  ...
  void operator()(const edm4hep::TrackCollection& col) const override {
    ...
    for (unsigned i = 0; i < col.size(); ++i) {
      ...
      for (unsigned j = 0, N = trk.getTracks().size(); j < N; ++j) {
        ...
        debug() << " -- track j= " << j << " has " << t.getTrackerHits().size() << " hits  " << endmsg;
      ...
      for (TrackerHitVec::const_iterator it = hits.begin(); it != hits.end(); it++) {
        ...
        ced_hit_ID((*it).getPosition()[0], (*it).getPosition()[1], (*it).getPosition()[2], marker, layer, size, color,
                   myColID + trk.id().index);
      }
      ...
      case 4: // AtCalo
        ...
        ced_hit_ID(ts.referencePoint[0], ts.referencePoint[1], ts.referencePoint[2], 1, layer, size * 20, color,
                   myColID + trk.id().index);
