...
double calculateTrackLength(const CalorimeterDrawParams& barrel, const CalorimeterDrawParams& endcap, double x,
                            double y, double z, double px, double py, double pz, double rel_X0) {

  if (barrel.delta_z == -1 || endcap.delta_z == -1)
    return 0; // the case if the parameters could not be loaded properly
  ...
  if (r > barrel.r_inner || r > endcap.r_inner)
    return 0;
  ...
  // case 2: barrel + endcap hit
    ...
    // x == path traversed in barrel, rotation symmetry is still assumed at this point which is a valid approximation
    // most of the times
