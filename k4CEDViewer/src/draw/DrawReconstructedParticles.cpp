...
struct DrawReconstructedParticles final : k4FWCore::Consumer<void(const edm4hep::ReconstructedParticleCollection&)> {
  ...
  Gaudi::Property<bool> drawHelixForPFOs{this, "drawHelixForPFOs", true,
                                         "draw a helix for ReconstructedParticle objects: -1: none, 0: default, 1: "
                                         "atIP, 2: atFirstHit, 3: atLastHit, 4: atCalorimeter"};

  Gaudi::Property<float> lineThickness{this, "lineThickness", 1., "line thickness for drawning helix  "};
  Gaudi::Property<bool> useColorForHelixTracks{
      this, "useColorForHelixTracks", 0,
      "draw helices in the color of charged reconstructedParticle/PFO: 0: disabled (lightgrey), 1: enabled "};
  ...
  Gaudi::Property<bool> colorByEnergy{this, "ColorByEnergy", false, "color recunstructed particle by energy"};

  Gaudi::Property<bool> colorByEnergyAuto{
      this, "ColorByEnergyAuto", false,
  ...
  void operator()(const edm4hep::ReconstructedParticleCollection& col) const override {
    ...
    info() << " +++++++  drawing ReconstructedParticle collection with " << col.size() << " particles "
    ...
    // Determine the maximal and minimal cluster energy depositions in the event for color scaling (-->when drawing
    ...
    for (int ip(0); ip < nelem; ++ip) {
      ...
      edm4hep::ReconstructedParticle thePart = col[ip];

      std::vector<edm4hep::ReconstructedParticle> pV;

      if (!thePart.getParticles().empty()) {
        std::copy(thePart.particles_begin(), thePart.particles_end(), std::back_inserter(pV));
      } else {
        pV.push_back(col[ip]);
      }
    ...
    for (int ip(0); ip < nelem; ++ip) {
      ...
      int thePartIndex = myColID + thePart.id().index; // need to comput the index of the parent particle
      ...
      if (!thePart.getParticles().empty()) {
        ...
        debug() << " --- draw particle " << ip << "  with " << pV.size() << " particles  : " << thePart << endmsg;
      ...
      for (auto part : pV) {
        ...
        if (colorByEnergy) {
          if (colorByEnergyAuto) {
            ...
            color = ColorMap::NumberToTemperature(ene, colorByEnergyMin, colorByEnergyMax, colorByEnergySaturation,
                                                  colorByEnergyValue);
          }
        }
        ...
        debug() << "Particle : " << ip << " PX = " << px << " PY = " << py << " PZ = " << pz << " E  = " << ene
                << endmsg;
        ...
        for (unsigned icluster = 0; icluster < nClusters; ++icluster) {
          ...
          for (int iHit = 0; iHit < nHits; ++iHit) {
            ...
            ced_hit_ID(x, y, z, marker, layer, size, color, thePartIndex);
          }
          ...
          if (nHits == 0) { // draw large marker at cluster position
            ...
            ced_hit_ID(x, y, z, marker, layer, 5 * size, color, thePartIndex);
          }
        }
        ...
        if (drawEllipsoidForPFOClusters) {
          ...
          for (unsigned int p = 0; p < nClusters; p++) {
            ...
            // For each cluster, it's (energy weighted) central position, the deposited energy and the intrinsic
            // direction in terms of sperical angles are given. The minimal and maximal deposited energies among all
            // clusters in the displayed event have been determined previously and will be needed for coloring.
            ...
            if (phi == 0. && theta == 0.) {
              theta = atan(sqrt(cluster_center[0] * cluster_center[0] + cluster_center[1] * cluster_center[1]) /
                           cluster_center[2]);
              phi = atan2(cluster_center[1], cluster_center[0]);
            }

            // Energy weighted moments of inertia are calculated. Ultimately, the eigenvalues of the 3x3 matrix will be
            // a measure of the ellipsoids' extensions.
            CalorimeterHitVec hitvec = cluster.getHits();
            int nHits = (int)hitvec.size();
            double Etot = 0;
            double I[3][3];
            for (int i = 0; i < 3; i++)
              for (int j = 0; j < 3; j++)
                I[i][j] = 0;
            // The angles theta and phi are used to transform the coordinates of each hit into a c.s. in which the
            // x'-axis is parrallel to the ellisoid's intrinsic direction. Note that this does not describe an
            // unambigious system as any rotation along the x'-axis does not touch this constraint. The transformation
            // is achieved by a typical multiplication of rotation matrices: R(theta, phi) = R_y(Pi/2 - theta)*R_z(phi)
            double R[3][3];
            R[0][0] = cos(phi) * sin(theta);
            R[1][0] = -sin(phi);
            R[2][0] = -cos(phi) * cos(theta);
            R[0][1] = sin(phi) * sin(theta);
            R[1][1] = cos(phi);
            ...
            for (int q = 0; q < nHits; q++) {
              ...
              // ced_hit_ID(x,y,z,marker, layer ,size,color,thePartIndex);   //this line draws the indivdual hits within
              // a cluster translation and rotation of the coordinates
              x -= cluster_center[0];
              y -= cluster_center[1];
              z -= cluster_center[2];
              tot_x += x * e;
            ...
            //   (component in coordinate system with x' || intrinsic direction)       (system in which ellipsoids are
            //   initially placed)
            //                           x'                                      <-->        z
            //                           y'                                      <-->        y
            //                           z'                                      <-->       -x
            // These assignments are corrected for by a modified rotation of the ellipsoid along its y-axis (see
            // declaration of double rotate[])
            ...
            if (nHits > 0) {
              ...
              double lambda1 = 0.5 * (I[2][2] + I[1][1]) +
                               sqrt(pow(0.5 * (I[2][2] + I[1][1]), 2) + pow(I[2][1], 2) - I[2][2] * I[1][1]);
              double lambda2 = 0.5 * (I[2][2] + I[1][1]) -
                               sqrt(pow(0.5 * (I[2][2] + I[1][1]), 2) + pow(I[2][1], 2) - I[2][2] * I[1][1]);
              sizes[0] = I[0][0];
              sizes[1] = lambda1;
              sizes[2] = lambda2;
              // Remaining: (more or less) Arbitrary rescaling and transformation to a physical length (sqrt + energy
              // division)
              for (int i = 0; i < 3; i++)
                sizes[i] = sqrt(17.727) * sqrt(sizes[i]) / Etot;
            ...
            int ellipsoid_color = color; //( colorByEnergy ? returnRGBClusterColor(cluster.getEnergy(), Emin, Emax, 256,
                                         //'a', 3) : color ) ;

            debug() << " draw ellipse for cluster : " << cluster.id().index << " - energy : " << cluster.getEnergy()
                    << " size: " << sizes[0] << ", " << sizes[1] << ", " << sizes[2] << " rotate: " << rotate[0] << ", "
                    << rotate[1] << ", " << rotate[2] << endmsg;
            ...
            // ced_geocylinder_r(0.25*(sizes[0]+sizes[1]), sizes[2], cluster_center, rotate, 36, ellipsoid_color,
            // layer);
          }
        }
        ...
        for (unsigned itrack = 0; itrack < nTracks; ++itrack) {
          ...
          for (unsigned j = 0, N = trk.getTracks().size(); j < N; ++j) {
            ...
            debug() << " -- track j= " << j << " has " << t.getTrackerHits().size() << " hits  " << std::endl;
  ...
  int returnRGBClusterColor(float eneCluster, float cutoff_min, float cutoff_max, int color_steps, char scale,
                            int colorMap) const {
    ...
    default: // log
      color_delta =
          (int)((log_ene - log_min) / log_step); // which colour bin does the value go to? We have [0x00,0xFF] bins
      break;
    case 'b': // linear
