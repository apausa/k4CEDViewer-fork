...
void drawDD4hepDetector(dd4hep::Detector& lcdd, bool _surfaces, StringVec _detailled) {
  ...
  for (unsigned i = 0, n = calorimeters.size(); i < n; ++i) {
    ...
    try {
      ...
      debug() << detName << " has no extension of type LayeredCalorimeterData. " << endmsg;
    }
    ...
    if (visible && !isDrawn) {
      if (calo != 0) {
        ...
        gTV.push_back(CEDGeoTube(params.Rmax, params.Rmin, params.outer_symmetry, params.inner_symmetry, params.phi0,
                                 params.delta_phi, params.delta_z, params.z0, color, layer, 1, 1));
        if (!params.isBarrel) {
          // place the second one symmetric to the z-axis. An additional shift by the width of the layer is needed since
          // the appropriate argument represents the left handed start of the geometry
          gTV.push_back(CEDGeoTube(params.Rmax, params.Rmin, params.outer_symmetry, params.inner_symmetry, params.phi0,
                                   params.delta_phi, params.delta_z, -(params.z0 + 2.0 * params.delta_z), color, layer,
                                   1, 1));
        }
        isDrawn = true;
      }
  ...
  for (unsigned i = 0, n = trackers.size(); i < n; ++i) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          debug() << detName << " has no extension of type ZPlanarData/ZDiskPetalsData. " << endmsg;
        }
      }
    }
    ...
    if (!isDrawn && visible) {
      ...
      if (trkPlanar) {
        for (std::vector<dd4hep::rec::ZPlanarData::LayerLayout>::iterator thisLayer = trkPlanar->layers.begin();
             thisLayer != trkPlanar->layers.end(); thisLayer++) {
          LayerGeometry Geo;
          ...
          if (detailledDrawing(_detailled, detName)) {
            for (unsigned stave_i = 0; stave_i < Geo.staves.size(); ++stave_i) {
              ced_geobox_r_ID(Geo.staves[stave_i].sizes, Geo.staves[stave_i].center, Geo.staves[stave_i].rotate, color,
                              layer, 0);
              ced_geobox_r_solid(Geo.staves[stave_i].sizes, Geo.staves[stave_i].center, Geo.staves[stave_i].rotate,
                                 color, layer);
            }
          } else {
            gTV.push_back(CEDGeoTube(Geo.tube.Rmax, Geo.tube.Rmin, Geo.tube.inner_symmetry, Geo.tube.outer_symmetry,
                                     Geo.tube.phi0, Geo.tube.delta_phi, Geo.tube.delta_z, Geo.tube.z0, color, layer, 1,
                                     1));
          }
        }
        isDrawn = true;
      ...
      if (trkDisk) {
        if (detailledDrawing(_detailled, detName)) {
          ...
          for (std::vector<dd4hep::rec::ZDiskPetalsData::LayerLayout>::iterator thisLayer = trkDisk->layers.begin();
               thisLayer != trkDisk->layers.end(); thisLayer++) {
            CEDGeoTubeParams params;
            params = PetalParameterConversion(thisLayer);
            gTV.push_back(CEDGeoTube(params.Rmax, params.Rmin, params.outer_symmetry, params.inner_symmetry,
                                     params.phi0, params.delta_phi, params.delta_z, params.z0, color, layer, 1, 1));
            // place the second one symmetric to the z-axis. An additional shift by the width of the layer is needed
            // since the appropriate argument represents the left handed start of the geometry
            gTV.push_back(CEDGeoTube(params.Rmax, params.Rmin, params.outer_symmetry, params.inner_symmetry,
                                     params.phi0, params.delta_phi, params.delta_z, -(params.z0 + 2 * params.delta_z),
                                     color, layer, 1, 1));
            k4GaudiCED::set_layer_description(detName, layer);
          }
        }
  ...
  for (unsigned i = 0, n = passiveDets.size(); i < n; ++i) {
    ...
    try {
      ...
      try {
        ...
        debug() << detName << " has no extension of type ConicalSupportData/LayeredCalorimeterData. " << endmsg;
      }
    }
    // get the visAttributes of the detElement's volume or (if not existing) some default values
    getVisAttributes(det, color, visible);
    ...
    if (!isDrawn && visible) {
      if (passiveConical) {
        ...
        for (std::vector<dd4hep::rec::ConicalSupportData::Section>::iterator thisSection =
                 passiveConical->sections.begin();
             thisSection != passiveConical->sections.end(); thisSection++) {
        }
        isDrawn = true;
      }
      if (passiveCalo) {
        for (std::vector<dd4hep::rec::LayeredCalorimeterData::Layer>::iterator thisLayer = passiveCalo->layers.begin();
             thisLayer != passiveCalo->layers.end(); thisLayer++) {
          CEDGeoTubeParams params;
...
CEDGeoTubeParams CalorimeterParameterConversion(dd4hep::rec::LayeredCalorimeterData* calo) {
  ...
  if (outer_symmetry < 1)
    outer_symmetry = NCircle;
  if (inner_symmetry < 1)
    inner_symmetry = NCircle;
  ...
  // respecting the rotation symmetry in 360/n, the minimal phi0 is calculated to prevent interference with phi cuts in
  // the CEDViewer phi0 == 0 implies a normal vector of the first cell parallel to the +x-axis but CED starts drawing
  // symmetrically from the +y-axis
  ...
  // in ILD and CLIC, both inner and outer shapes agree. For a generic solution, another parameter in
  // LayeredCalorimeterData (e.g. phi_inner) should be introduced i.e. delta_phi that is the rotational angle of the
  // inner angle with respect to the outer layer
  returnParams.delta_phi = -returnParams.phi0 + inner_phi0 + 270. - 180. / inner_symmetry;
  returnParams.delta_phi =
      returnParams.delta_phi - (360. / inner_symmetry) * (int(returnParams.delta_phi / (360. / inner_symmetry)));
...
CEDGeoTubeParams PetalParameterConversion(std::vector<dd4hep::rec::ZDiskPetalsData::LayerLayout>::iterator thisLayer) {
  ...
  */
  ...
  returnParams.phi0 = phi0 + 270. - 180. / petalNumber -
                      (360. / petalNumber) * (int((270. - 180. / petalNumber) / (360. / petalNumber)));
...
LayerGeometry TrackerLayerParameterConversion(std::vector<dd4hep::rec::ZPlanarData::LayerLayout>::iterator thisLayer) {
  ...
  Geometry.tube.phi0 =
      phi0 + 270. - 180. / nLadders - (360. / nLadders) * (int((270. - 180. / nLadders) / (360. / nLadders)));
  Geometry.tube.delta_phi = 0.0;
  Geometry.tube.delta_z = zHalf_sensitive;
  Geometry.tube.z0 = -zHalf_sensitive;

  // return both possible sets of parameters describing the geometry. The choice for either is implemented in the main
  // draw routine
  return Geometry;
}
    ...
    for (SMap::const_iterator it = sMap->begin(); it != sMap->end(); ++it) {
      ...
      for (unsigned i = 0; i < lines.size(); i++) {
        ...
        ced_line_ID(lines[i].first.x() / dd4hep::mm, lines[i].first.y() / dd4hep::mm, lines[i].first.z() / dd4hep::mm,
                    lines[i].second.x() / dd4hep::mm, lines[i].second.y() / dd4hep::mm,
                    lines[i].second.z() / dd4hep::mm, default_type, default_width, color, layer);
        lineCounter++;
      }
    }
  }
